  <?php
   ...
   //баланс
    ...
    if ($balance < $trans_summa) {
      $trans_mess = 'Ошибка! Недостаточно средств';
    }
  ...
  // - уменьшение баланса у отправителя
   ...
   if ($balance < $tr_s) {
     echo '<script>alert("Ошибка! Недостаточно средств.");
     self.location=("bank.php#windTrans");
     </script>';
     $tr_s = 0;
     $trans_mess = 'Ошибка! Недостаточно средств.';
   }
  ...
  // + увеличение баланса у получателя
   ...
   mysqli_close($mysql);
   header('Location: bank#windPenalt.php');
?>
